...
class TestBusinessRules(TestSetUp):
    ...
    def test_cannot_schedule_conflict_same_professional(self):
        ...
        horario_comum = (timezone.now() + timedelta(days=10)).replace(minute=0, second=0, microsecond=0)
        ...
        res = self.client.post(self.consulta_url, payload_conflito, format='json')
