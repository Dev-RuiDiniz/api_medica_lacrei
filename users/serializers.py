...
class ProfissionalSerializer(serializers.ModelSerializer):
    profissao_display = serializers.CharField(
        source='get_profissao_display', read_only=True
    )
...
class ConsultaSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        data_hora = data.get(
            'data_hora', getattr(self.instance, 'data_hora', None)
        )
        profissional = data.get(
            'profissional', getattr(self.instance, 'profissional', None)
        )
        ...
        if data_hora and data_hora < timezone.now():
            raise serializers.ValidationError(
                {'data_hora': 'A data da consulta não pode ser no passado.'}
            )
